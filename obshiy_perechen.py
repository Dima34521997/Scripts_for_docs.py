...
comment_category = {}  # Категории примечаний
comment_not_install = {}  # Категории для "Не устанавливать"
...
def get_dfs(dict_chars: dict, files: list):
    ...
    if len(files) == 0:
        ...
        for file in files:
            ...
            if no_perechen == 1:
                b = dfs[i][dfs[i]['Примечание'] != 'Не устанавливать']
                dfs[i] = b
...
def get_components(dict_chars: dict, dfs: list, files: list):
        ...
        for ind, df in enumerate(dfs):
            ...
            while df_index < len(df.index):
                ...
                if '*' in component.desig:
                    if 'Регулирование' not in comment_category.values():
                        comment_category['*' * (len(comment_category) + 1)] = 'Регулирование'
    ...
    if len(errors) != 0:
        ...
        for error in errors:
            print(error)
        print("*** *** ***\n")
    return dict_chars, comment_not_install, comment_category, components_one_manuf_categories


# def add_to_comment(chip: ed.Element, char):
#     global comment_not_install
#     if chip.comment != '' and "*" not in chip.comment:
#         if chip.comment.lower() == 'не устанавливать':
#             if 'не устанавливать' not in comment_category.values():
#                 comment_category['*' * (len(comment_category) + 1)] = 'не устанавливать'
#             if char not in comment_not_install.keys():
#                 comment_not_install[char] = []
#             if chip.desig not in comment_not_install[char]:
#                 comment_not_install[char].append(chip.desig)


# def add_to_comment(chip: ed.Element, char):
#     global comment_not_install
#     if not chip.comment and '*' in chip.comment:
#         return
#     if chip.comment.lower() != 'не устанавливать':
#         return
#     if 'не устанавливать' not in comment_category.values().lower():
#         comment_category['*' * (len(comment_category) + 1)] = 'не устанавливать'
#     if char not in comment_not_install.keys():
#         comment_not_install[char] = []
#     if chip.desig not in comment_not_install[char]:
...
def combine_following_chips(component: ed.Element, df: pd.DataFrame, df_index: int):
    ...
    while True:
        ...
        add_to_comment(next_component, next_component.char)
        same += 1
            ...
            if len(splited_values) != 1:
                for ind, value in enumerate(splited_values):
                    if ind == 0:
                        ...
                        d_chars[char][d_chars_index].comment = '*'
                    else:
                        line = copy.copy(chip)
                        line.val = value
                        line.comment = '*'
