...
class UI(Frame):
    def __init__(self):
        ...
        self.btn_AddFile = Button(self, text="Добавить файл/-ы",
                                  command=self.onOpen)
        self.btn_PathToTemplate = Button(self, text='Путь к шаблону', command=self.onOpenTemplatePath)
    ...
    def initUI(self):
        ...
        self.btn_AddFile.grid(column=0, row=4,
                              padx=2, pady=2)
        self.btn_PathToTemplate.grid(column=1, row=5,
                               padx=2, pady=2)
    ...
    def onOpenTemplatePath(self):
        ...
        if path:
            self.ent_Templates.insert(END, path.replace('/', '\\'))
