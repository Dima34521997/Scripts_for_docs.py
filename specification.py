    ...
    try:
        ...
        for char in sorted(dict_chars.keys()):
            for chip_index, chip in enumerate(dict_chars[char]):
                ...
                chip.split_desig(shift_treshold=10)
...
def execute(input_files: list, is_civ: bool):
    ...
    for file in files:
        ...
        dfs, files = obshiy_perechen.get_dfs(d_chars, [file])
        print("Формирую общую таблицу элементов...")
        d_chars, prim_not_install, prim_cats, one_man_cats = obshiy_perechen.get_components(d_chars, dfs, [file])
